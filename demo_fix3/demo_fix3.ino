...
// Current spherical harmonic type - Now defined by l and m, and button cycles orbitals
int current_l = 0;
int current_m = 0;

// Define a list of orbitals (l, m pairs) to cycle through with the button
struct Orbital {
  int l;
  int m;
  const char* name; // Optional name for display
};

const Orbital orbitals[] = {
    {0, 0, "Y(0,0) - s"},       // s orbital
    {1, 0, "Y(1,0) - p_z"},     // p orbitals
    {1, 1, "Y(1,1) - p_x/y"},
    {2, 0, "Y(2,0) - d_z^2"},    // d orbitals
    {2, 1, "Y(2,1) - d_xz/yz"},
    {2, 2, "Y(2,2) - d_x^2-y^2/xy"},
    {3, 0, "Y(3,0) - f_z^3"},    // f orbitals (example)
    {3, 1, "Y(3,1) - f_xz^2/yz^2"},
    {3, 2, "Y(3,2) - f_xyz/z(x^2-y^2)"},
    {3, 3, "Y(3,3) - f_x^3/y^3"}
};
const int num_orbitals = sizeof(orbitals) / sizeof(orbitals[0]);
int current_orbital_index = 0;
...
void setup() {
  ...
  // Setup button for orbital change
  pinMode(BUTTON_PIN, INPUT_PULLUP);

  // Initialize current orbital from the list
  current_l = orbitals[current_orbital_index].l;
  current_m = orbitals[current_orbital_index].m;
  ...
  tft.setCursor(5, 20);
  tft.println("Press button to change Orbital"); // Button now changes Orbitals
  ...
  Serial.println("----------------------------------------------------");
  Serial.println("Button cycles through predefined orbitals.");
  Serial.println("Use serial commands 'l' and 'm' for direct control.");
  Serial.println("Current parameters:");
  Serial.print("  Orbital: "); Serial.println(orbitals[current_orbital_index].name);
...
void loop() {
  ...
  // Check button for orbital change (cycles through predefined orbitals)
  int reading = digitalRead(BUTTON_PIN);
  if (reading != lastButtonState && millis() - lastDebounceTime > debounceDelay) {
    if (reading == LOW) { // Button pressed
      current_orbital_index = (current_orbital_index + 1) % num_orbitals; // Cycle through orbitals
      current_l = orbitals[current_orbital_index].l;
      current_m = orbitals[current_orbital_index].m;
      // Clear screen for new orbital
      tft.fillRect(0, 35, SCREEN_WIDTH, SCREEN_HEIGHT - 35, BACKGROUND_COLOR);
      Serial.print("Orbital changed by button to: "); Serial.println(orbitals[current_orbital_index].name);
      Serial.print("l="); Serial.print(current_l); Serial.print(", m="); Serial.println(current_m);
    }
  ...
  // Display current orbital info
  tft.setCursor(5, 35);
  tft.print("Orbital: "); tft.println(orbitals[current_orbital_index].name); // Display orbital name
  tft.setCursor(5, 50); // Move cursor down a bit
    ...
    if (equalSignIndex != -1) {
      ...
      if (parameter.equalsIgnoreCase("l")) {
        ...
        if (value >= 0 && value <= 10) { // Example upper limit for l, adjust if needed
          ...
          current_orbital_index = -1; // Invalidate orbital index for button cycle to restart from list beginning if button is used after serial l/m input
